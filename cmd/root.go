...
func createRootCommand(ctx context.Context, input *Input, version string) *cobra.Command {
	...
	rootCmd.Flags().BoolVar(&input.tryFromGithubComOnCloneFail, "try-from-github-com-on-clone-fail", false, "If clone fails, try from GitHub (github.com) instead. Only works with --github-instance")
...
func newRunCommand(ctx context.Context, input *Input) func(*cobra.Command, []string) error {
	return func(cmd *cobra.Command, args []string) error {
		...
		config := &runner.Config{
			...
			TryFromGithubComOnCloneFail:        input.tryFromGithubComOnCloneFail,
			Matrix:                             matrixes,
